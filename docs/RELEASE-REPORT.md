...
| 双审最终教案 | 教研批准后自动打开，6 个内容章节完整可见 |
| 审核持久化 | `approved` 状态与 3 条审核日志自动保存 |
| 桌面与移动端 | 1440px、390px 均无横向溢出，运行时异常 0 |
...
6. 双审后独立最终教案与“查看最终教案”入口。
7. 审核步骤自动保存，浏览器本地草稿箱可恢复。
8. Markdown 与 JSON 导出。
9. 响应式网页与隐私提示。
